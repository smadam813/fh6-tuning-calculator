...
| **Gearing** | `FinalDrive = 4.25 + (400 − hp)/600` (+ weight, goal, aero, drivetrain, engine-location terms). With a redline + target top speed, the final drive is instead back-solved from physics — but against an **effective top-speed RPM**, not the redline: top speed is power-limited and FH6 power fades near the limiter, so the engine tops out *below* redline. That rpm is the supplied **peak-power RPM** (read off the dyno), capped by a droop-aware estimate (`≈ 0.983 · 5252 · hp/torque`) so an above-redline peak can't over-shorten the gearing; the estimate alone is used when peak-power RPM is blank. Gear ratios follow the power series `Rₙ = A · nᴮ` with first gear `A` from power-to-weight; the spacing exponent `B ≈ −0.65` is **powerband-aware** when a **max-torque RPM** is supplied — sized from the band width `redline / maxTorqueRPM` so each upshift drops the engine back toward its torque band (wider band → wider gaps, narrower band → closer gears), clamped to stay strictly descending and on the slider for any gear count 2–10. For a *narrow* band the tight spacing would otherwise stretch first gear too tall to launch, so the gearbox goes **progressive**: 1st is anchored to a launchable overall ratio and the per-shift gaps **taper smoothly** (widest at 1→2) down to the close-ratio cluster at the top, so every upshift lands back near the torque band instead of dumping the whole launch span into one bog-inducing 1→2 jump (wide bands keep the plain power series). EVs are single-speed: the lone "1st" ratio is shown alongside the final drive (both are in-game sliders and only correct as a pair), and target-top-speed solves gear the limiter ~7% past the target because FH6 EV motors lose power near redline. |
