...
| **Springs (F/R rate)** | `K = f²·Wcorner/9.78` (f from class+aero+goal+dt split); weight-dist fine-tune; EV×1.08/Hybrid×1.04; support floor; clamp per-axle part min/max (independent front/rear ranges) | spec-springs-damping §1 | ✔ part min/max | ✔ | None |
| **Ride Height (F/R)** | `ride = rhMin + (rhMax−rhMin)·(base[goal]+comp+aero+softFront+heavy)`; clamp per-axle min/max (independent front/rear ranges), round 0.1 | spec-springs-damping §2 | ✔ part min/max | ✔ | Front/rear part ranges now independent (2026-06); drivetrain-insensitive by design |
